...
export interface TooltipProps extends Omit<HTMLAttributes<HTMLDivElement>, 'children'> {
    ...
    /**
     * When `true`, the tooltip never renders regardless of hover / focus.
     * Use to suppress the tooltip while a coupled menu/dropdown is open
     * (e.g. an icon button that also opens a `<DropdownMenu>` — once the
     * menu opens, hide the tooltip so they don't fight for the same
     * screen space). Flipping this to `true` mid-render immediately
     * force-closes any currently visible tooltip.
     */
    isDisabled?: boolean;
    /** Trigger element(s). Single React element preferred (the trigger receives `aria-label`). */
    children: ReactNode;
}
export declare function Tooltip({ bodyText, heading, placement, open: controlledOpen, onOpenChange, zIndex, accessibilityLabel, isDisabled, children, className, ...props }: TooltipProps): import("react/jsx-runtime").JSX.Element;
